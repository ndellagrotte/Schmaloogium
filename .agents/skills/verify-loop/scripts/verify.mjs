...
import { DEFAULT_HEARTBEAT_MS, PROGRESS_MODES, progressSinks } from "./progress.mjs";
...
Options:
  ...
  --model <model>          Optional model pattern passed to each omp role session.
  --agent-timeout-ms <n>   Per-agent timeout in milliseconds (default: 1800000).
  --progress <mode>        Live progress on stderr: human (default), json, or none.
  --heartbeat-ms <n>       Heartbeat interval while a stage is in flight (default: 30000; 0 off).
...
every role as one fresh, isolated omp agent session (createAgentSession). Read-only roles get a
read/grep/glob tool allowlist; writer roles add write/edit behind a deterministic pre-write path
hook, with post-stage worktree-hash allowlist and immutable-evidence checks as the backstop.
Different targets may run in parallel; their mutation windows queue, and duplicate runs of one
target fail with RUN_CONFLICT.

Live runs require Bun plus the pinned @oh-my-pi/pi-coding-agent SDK (17.2.11; override discovery
with OMP_SDK_PATH). Dry runs, resolution, and the test suite run under plain Node.
Progress is written to stderr as it happens and appended to a timestamped .log beside the run
journal, so a run is tailable from another terminal while the JSON result stays on stdout.
`;
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--quiet":
        ...
        break;
      case "--heartbeat-ms":
        options.heartbeatMs = parseInteger(arg, argv[++index]);
        break;
      case "--design-version":
  ...
  if (!PROGRESS_MODES.has(options.progress)) {
    throw new VerificationError(`--progress must be one of ${[...PROGRESS_MODES].join(", ")}`);
  }
  if (!Number.isSafeInteger(options.heartbeatMs) || options.heartbeatMs < 0) {
    throw new VerificationError("--heartbeat-ms must be an integer >= 0");
  }
  return options;
}
...
async function main() {
  ...
  const runOptions = { ...options, ...progressSinks(options.progress) };
  let result;
  if (options.fixupReview) {
    result = options.dryRun
      ? dryRunFixupPlan(contract, options.fixupReview)
      : await executeFixupContinuation(contract, runOptions);
  } else {
    result = options.dryRun || options.maxRounds === 0
      ? dryRunPlan(contract, options)
      : await executeVerification(contract, runOptions);
  }
