...
**Date:** 2026-08-08 · **Session type:** build (§G1.1).
...
|---|---|---|
| `docs/phase1/v14/PHASE_1_DOC.md` | **verified** — `docs/phase1/reviews/PHASE_1_REVIEW_25.md` l. 61 literal `PASS`, `Interface changed: no` | §2.1 package tables (ll. 1527–1563), §4.9.4 diagnostics (ll. 3776–3797), §4.2.6/§4.8.4 dependency mechanics as reached from §5.3, §5 whole (ll. 4175–4281), §10.2 OQ-12 (ll. 4580–4610), §11.3 item 2 and §11.4's "To Phase 12" block (ll. 5167–5171), §9 row for `SHADER_GUI` (l. 4513) |
| `docs/phase3/v1/PHASE_3_DOC.md` | **verified** — `docs/phase3/reviews/PHASE_3_REVIEW_34.md` l. 46 literal `PASS`, `Interface changed: no` (l. 48) | §2.2 public shape (ll. 326–535), §3.1 flag-ownership map (ll. 679–709), §3.2 rows for sliders/profiles/screens (ll. 731–734, 752), §4.3 (ll. 895–943), §4.8 (ll. 1180–1236), §5 whole (ll. 1394–1632), §6 (ll. 1634–1649), §7 (ll. 1651–1672), §11.4 (ll. 1959–1979), §11.5 (ll. 1981–1986) |

### 0.3 Phase 7 — the sanctioned soft-dependency exception

Phase 12's dependency on Phase 7 is **soft** (DESIGN.md ll. 630–632). `PHASE_7_DOC.md` is
**not verified**: `docs/phase7/reviews/PHASE_7_REVIEW_32.md` l. 299 records
`PASS-WITH-CORRECTIONS` with `Interface changed: yes`, so under §G1.3 it owes a fresh verify
session before a dependent may consume it, and under the §G5.3 gating invariant it is not a valid
dependency input today.

This session therefore invokes **§G5.3 item 3, the one sanctioned exception**: it designs against
Phase 7's *specification in DESIGN.md* (ll. 1850–1886), does **not** read `PHASE_7_DOC.md`, and
flags every consumed assumption in §5.3. If Phase 7's verified doc later contradicts an assumption
there, this doc takes a §G1.3 **fix-up** session, not a rebuild.
...
| Persistence **formats** and both codecs (`OptionPersistenceCodec`, `GlobalShaderOptionsCodec`), ISO-8859-1 escaping, atomic write mechanics | **Phase 3** (§4.3 ll. 926–932). We own only *when* they are invoked (P3 §4.3 l. 934) |
| `ScreenModel`/`ProfileModel`/`SliderSet`/`LanguageMap` parsing and the column formula itself | **Phase 3** (§3.2 ll. 731–734, §4.3 ll. 938–943) |
| Profile inference algorithm (`ProfileModel.infer`) | **Phase 3** (§4.8 ll. 1228–1231) |
...
| Behavior of each engine setting: `normalMap`/`specularMap` → **Phase 13**; `renderQuality` → **Phase 5** sizing with **Phase 7** viewport; `shadowQuality` → **Phase 8**; `handDepth` → **Phase 7**; `oldHandLight` → **Phase 9**; `oldLighting` → **Phase 10** (the last two per P3 §3.1 ll. 689, 691) | as listed |
...
| F3-3 | Recognized only when the same file `#ifdef`/`#ifndef`s it | Not re-checked: the presentation model is built from P3's confirmed catalog only (P3 §4.3 ll. 901–902) | P3 §4.3 |
| F3-4 | Tooltips split on `". "` | `TooltipBuilder.split` produces one `TooltipLine` per segment (§4.3.5) | `[V:doc]` App F.3 l. 1457 |
| F3-5 | Lines ending `"!"` render **red** | `TooltipSeverity.WARNING` → red in both views; P3 sets the severity (§4.3 l. 914), Phase 12 chooses the colour (§4.3.5) | `[V:doc]` App F.3 l. 1458; P3 §4.3 l. 914 |
...
| F3-10 | Ambiguous options (conflicting defaults) are **disabled** | Rendered with `interactive=false`, label suffixed with the ambiguity marker, tooltip carrying P3's reported locations; excluded from `*` expansion (§4.3.3, `[D-P12-4]`) | `[V:doc]` App F.3 l. 1467; P3 §4.3 ll. 909–911, §6 l. 1640 |
...
| F3-20 | `sliders=<option list>` renders listed **variable** options as sliders | `SliderOption` emitted instead of `ValueOption` for members of P3's `SliderSet`; unknown/non-variable entries are diagnosed by P3 and simply do not become sliders (§4.4.3, `[D-P12-5]`) | `[V:doc]` App F.3 l. 1470; P3 §3.2 l. 731 |

### 3.2 RESEARCH Appendix F.4 — profiles and screens

| # | Contract item (App F.4, ll. 1472–1480) | Design element | Provenance |
|---|---|---|---|
| F4-1 | `profile.NAME=` tokens `OPTION` / `!OPTION` / `OPTION:value` / `OPTION=value` | Applied as a batch into the pending set on profile click (§4.5.2), from P3's expanded constraint list | `[V:doc]` App F.4 ll. 1474–1475; P3 §3.2 l. 732 |
| F4-2 | `profile.OTHER` copy, cycle-guarded | Consumed post-expansion; P3 ignores the cyclic edge and diagnoses (§6) | `[V:doc]` App F.4 l. 1475; P3 §4.8 l. 1223 |
| F4-3 | `!program.<name>`, optionally dimension-prefixed | **Not actuated here** — Phase 4 consumes disabled programs. Phase 12 applies only the *option* constraints of a profile and never mutates program state (§4.5.2) | `[V:doc]` App F.4 ll. 1475–1476; P3 §3.2 l. 732 |
| F4-4 | Current profile inferred from option values; otherwise **"Custom"** | `ProfileCycle.current` = `ProfileModel.infer(pendingOptionState)`, evaluated against the **pending** state so the label tracks unapplied edits (§4.5.1) | `[V:doc]` App F.4 ll. 1476–1477; P3 §4.8 ll. 1228–1231 |
...
| F4-11 | Entry: `*` — all unplaced options | **Phase 12's** deferred expansion, specified completely in §4.3.3 (`[D-P12-1]`, `[D-P12-2]`, `[D-P12-3]`) | `[V:doc]` App F.4 l. 1479; deferred to us by P3 §4.3 ll. 941–943 and §4.8 ll. 1225–1226 |
| F4-12 | `screen[.NAME].columns=N`, default 2 | Explicit valid positive `N` wins; absent → P3's formula (§4.3.4) | `[V:doc]` App F.4 l. 1480; P3 §4.3 ll. 938–940 |
| F4-13 | Auto-widens beyond 18 options | `max(2, ceil(actualOptionCount / 9))`, counted **after** `*` expansion and excluding navigation/layout entries: 0–18 → 2, 19–27 → 3 (§4.3.4) | `[V:doc]` App F.4 l. 1480; P3 §4.3 ll. 940–943 |

### 3.3 Remaining in-scope contract rows

| # | Contract item | Design element | Provenance |
|---|---|---|---|
| C-1 | App F.2 `version.<mcver>=<edition>` → pack-list warning | Candidate row renders `CompatibilityStatus.REQUIRES_NEWER_EDITION` as a warning badge + tooltip; the pack stays off (§4.6.1) | `[V:doc]` App F.2 l. 1452; P3 §4.8 ll. 1185–1188, §6 l. 1643 |
| C-2 | §4.7 "only changed options persist to `shaderpacks/<pack>.txt`" | `apply()` writes exactly the changed set through `OptionPersistenceCodec`; Phase 12 computes the change set, Phase 3 owns the format (§4.7.2) | `[V:observed]` §4.7 ll. 603–605; P3 §4.3 ll. 926–932 |
| C-3 | §4.7 "global engine settings in `optionsshaders.txt`" | The seven entries round-trip through `GlobalShaderOptionsCodec` → `EngineOptionData` (§4.6.3) | `[V:observed]` §4.7 l. 605; P3 §5.1 ll. 1552–1555 |
...
| C-9 | §4.7 failure handling: invalid programs produce a user-visible error | The `SHADER_GUI` store is rendered by §4.9; Phase 4 emits, Phase 1 stores, we display | `[V:observed]` §4.7 ll. 612–614; P1 §4.9.4 l. 3790, §6 l. 4294 |
| C-10 | §4.7 failure handling: capability gate → chat error | Not ours — `UserChannel.CHAT`. The GUI additionally shows the resulting shaders-off state (§4.9) | `[V:observed]` §4.7 l. 615; P1 §4.9.4 l. 3797 |
...
| C-15 | §G4.1: pack-facing vocabulary used verbatim | `profile`, `screen`, `slider`, `<empty>`, `*`, `columns`, `option`, `value`, `prefix`, `suffix` all appear unrenamed in the model's type and field names (§2.2) | DESIGN.md ll. 545–550 |
...
| `(internal)` pack entry | **No reference.** Pintonium hardcodes `isInternal()` false (PD §7.1) and its list is a plain directory scan (ll. 230–248). We render P3's `PackCandidateKind.INTERNAL` sentinel from `discover` (§4.6.1) | PD §7.1; P3 §5.1 ll. 1526–1527 |
...
- `PackConfiguration.options()` — Phase 3's `OptionConfiguration`: the `OptionCatalog`, the
  immutable `OptionState`, and the profiles / screens / sliders / lang models (P3 §5.1 l. 1410);
- `PackConfiguration.compatibility()` and `PackConfiguration.pack()` — for the selection screen's
  status row (P3 §5.1 l. 1406).
...
- **I-1 — no re-parsing.** Phase 12 never opens a pack file, never rescans directives, never
  reinterprets properties, and never bypasses the materializer. P3 §5.1 l. 1572 states this as a
...
- **I-2 — no mutation.** `PackConfiguration` and `OptionState` are deeply immutable (P3 §7
  l. 1658). An edit produces a *pending overlay*, never an in-place change. A reload publishes a
  new configuration (P3 §5.1 l. 1573).
- **I-3 — schema and fingerprint gating.** Derived state (a built presentation model) is retained
  ...
  derive it (P3 §5.1 ll. 1573–1575). The model carries both and is rebuilt, never patched, when
  either changes. `PackFrontEnd.CURRENT_SCHEMA_VERSION` is `3` (P3 §5.3 l. 1593); any other value
  is rejected before a model is built.
...
1. Reject the configuration if `schemaVersion != 3` (I-3).
...
(P3 §6 l. 1641). The navigation stack is additionally depth-bounded at 32 as a defence in depth;
...
(P3 §4.8 ll. 1225–1226), and *"Phase 12 retains ownership of deferred `*` expansion; options
produced by that expansion do count before Phase 12 evaluates the formula"* (P3 §4.3 ll. 941–943).
...
ambiguous**. Ambiguous options are disabled by Phase 3 (P3 §4.3 ll. 909–911); auto-placing a
...
already uses for published collections (P3 §5.1 l. 1465). I-4 requires *some* total order; this one
...
formula runs (P3 §4.3 ll. 941–943) — see §4.3.4.

#### 4.3.4 Column resolution

Per App F.4 l. 1480 and P3 §4.3 ll. 938–940:
...
lang files and leaves the fallback choice to us (P3 §4.3 ll. 914–916).
...
sets that severity at parse time (P3 §4.3 l. 914) and both views render `WARNING` lines **red**
...
value to the UI's advertised list" (P3 §4.3 l. 930). A pack option can therefore legitimately hold
...
- `sliders=` naming an unknown or non-variable option is diagnosed by Phase 3 (P3 §3.2 l. 731) and
...
(P3 §4.8 ll. 1228–1231). Phase 12 re-implements none of it.
...
write-on-every-click: RESEARCH §4.7 l. 604's "write-through on change" is satisfied at apply time,
...
selection UI (P3 §5.1 ll. 1534–1535). Consumed properties, all of them Phase 3's:

- Result ordering is already correct: `Off` and `Internal` first, then filesystem candidates in
  Phase 3's deterministic order (P3 §5.1 l. 1526). **Phase 12 does not re-sort.**
- `PackCandidateKind` is closed — `OFF`, `INTERNAL`, `DIRECTORY`, `ARCHIVE` (P3 §5.1 l. 1527) — and
  each gets a distinct row icon/prefix.
- `PackCandidateStatus` is closed — `AVAILABLE`, `UNREADABLE`, `UNSAFE`, `LIMIT_EXCEEDED`
  (P3 §5.1 l. 1528). Non-`AVAILABLE` candidates render disabled with the status and the candidate's
  attributed diagnostics in the tooltip.
- Display names are sanitized by Phase 3 and are **never** accepted back as paths (P3 §5.1
  l. 1529). The view keeps them as display strings only.
- `PackCandidateId` is opaque and is valid only against the latest discovery generation for that
  directory; a later discovery supersedes it (P3 §5.1 ll. 1530–1533). The selection screen holds
...
- A `LOAD` that fails returns exactly one `Failed` and leaves the caller in shaders-off (P3 §5.1
  l. 1570). The screen shows the sanitized summary and stays open with `(off)` selected.
...
(P3 §4.8 ll. 1185–1188, §6 l. 1643), so the user can read the reason rather than face a silent
no-op (row C-1).
...
belongs to another phase, and this table is the authoritative statement of that split.
...
| Old hand light | `oldHandLight` | `TriState` | `DEFAULT` | **Phase 9** (P3 §3.1 l. 689) |
| Old lighting | `oldLighting` | `TriState` | `DEFAULT` | **Phase 10** (P3 §3.1 l. 691) |

**The tri-state is the documented priority rule made explicit** `[D-P12-10]`. App F.1 l. 1448
states that "corresponding in-game video settings win where both exist", and P3 §3.1 l. 684 assigns
...
#### 4.6.3 Global persistence

The seven settings round-trip through `GlobalShaderOptionsCodec` into the `EngineOptionData` that
`PackLoadRequest` already carries (P3 §2.2 l. 383, §5.1 ll. 1552–1555). Two consequences from
Phase 3's stated behavior:

- `EngineOptionData` is a stable-order map of **decoded string values**; unknown safe keys are
  retained for round-trip with a warning, and malformed entries are ignored with a warning. So the
  seven keys travel as strings and each behavior owner parses its own domain. Phase 12 does not
  need Phase 3 to know the key set, and no interface change is requested for this.
...
(P3 §4.3 ll. 926–932). *"Phase 3 owns codecs and file models. Phase 12 owns when a user applies/
discards and invokes writes"* (P3 §4.3 l. 934), and *"Phase 12 must define apply/discard timing and
global setting UX without changing the codecs"* (P3 §11.4 l. 1977). This section is the answer to
exactly that.
...
the edit (P3 §6 l. 1645; §6 row 6 below).
...
`SourceMaterializer.materialize` takes `OptionState` as an input (P3 §2.2 ll. 523–529). A changed
...
|---|---|---|---|---|
| **F3+R keybind** | `FULL` | if bake-set changed | no | Author-facing: re-reads the pack from disk, so discovery must re-run (row C-8) |
| **`/reloadShaders`** | `FULL` | if bake-set changed | no | Identical semantics to F3+R by contract (RESEARCH §4.7 l. 611) |
...
`VertexRequirements` is closed over exactly those three attributes (P3 §5.1 l. 1494), and
`separateAo`/`oldLighting` are the two App F.1 flags whose effect is baked into chunk geometry
(behavior owners Phase 10 / Phase 10, P3 §3.1 ll. 691, 703). The reference reloads renderers on
...
(P3 §11.5 item 2), the flag terms cannot change on an option-only apply, so today the predicate
...
- **RS-2 — never mid-frame.** The drain happens at a frame boundary, before Phase 7's frame-begin
  sequence. Publication is an atomic reference swap (P3 §7 l. 1659), so no consumer ever observes a
...
`cause = COMMAND` and replies with the outcome. Registration happens once during client init; the
command is client-side only, consistent with the client-only mission (§1.2 l. 78).
...
| `EngineSettingsModel`, `EngineSettingEntry` (`Toggle`/`TriState`/`Choice`), `TriStateValue` | The seven engine settings with their **stable key names, domains and defaults** — the table in §4.6.2 is the binding statement. Values travel to consumers as decoded strings inside `EngineOptionData`; each behavior owner parses its own key. `TriStateValue.DEFAULT` means "defer to the pack's App F.1 flag"; `ON`/`OFF` mean "the in-game setting exists and wins" (App F.1 l. 1448) | **13** (`normalMap`, `specularMap`), **5**+**7** (`renderQuality`), **8** (`shadowQuality`), **7** (`handDepth`), **9** (`oldHandLight`), **10** (`oldLighting`) |
| `PackSelectionModel`, `PackSelectionActions` | The selection screen's view model over `PackDiscoveryResult` — kind badge, status, compatibility badge, per-candidate diagnostics, current selection — plus the intents a view reports back. Contains no path, no root, no archive lease, and no unsanitized name (P3 §5.1 ll. 1529, 1533) | the view adapters |
...
### 5.2 Consumed from Phase 3 (`docs/phase3/v1/PHASE_3_DOC.md`, verified)

| Phase 3 §5.1 row | Use here |
|---|---|
| `PackFrontEnd.discover` / `PackDiscoveryRequest` / `PackDiscoveryResult` / `PackCandidate` (l. 1403) | Populates and refreshes the selection list (§4.6.1). Consumed properties: the `Off`/`Internal`-first ordering (l. 1526), closed `PackCandidateKind` (l. 1527), closed `PackCandidateStatus` (l. 1528), sanitized display names never accepted back as paths (l. 1529), and opaque `PackCandidateId` valid only against the latest generation (ll. 1530–1533) |
| `PackFrontEnd` / `PackLoadRequest` / `PackLoadResult` (l. 1404) | Every `REPUBLISH` and `FULL` lifecycle is a `load`. `PackSelection` is closed to `Off`/`Internal`/`Filesystem(id)` (l. 1538); `Off` returns `PackLoadResult.Off` without touching other request fields (l. 1541) |
| `PackConfiguration` (l. 1405) | The single input from which the presentation model is derived; consumed with its `schemaVersion == 3` gate and fingerprint retention rule (§5.3 l. 1593, §5.1 ll. 1573–1575) |
| `PackIdentity`, `CompatibilityStatus`, `DimensionConfiguration` (l. 1406) | Selection-screen status row; `REQUIRES_NEWER_EDITION` drives the `version.<mcver>` warning (row C-1) |
| `OptionConfiguration` — `OptionCatalog`, immutable `OptionState`, profiles/screens/sliders/lang (l. 1410) | The whole of §4.3–§4.5. The row explicitly assigns us the `*` expansion and states that expansion-produced options count toward widening |
| `OptionPersistenceCodec`, `GlobalShaderOptionsCodec` (l. 1411) | §4.7.2. Invoked, never modified — P3 §11.4 l. 1977 |
| `PackLoadRequest.engineOptions` / `EngineOptionData` (§2.2 l. 383, §5.1 ll. 1552–1555) | The seven engine settings' transport: stable-order decoded strings, unknown safe keys retained for round-trip, malformed entries warned and ignored |
| `ResourceRequirements.programs[*].vertices` / `VertexRequirements` (l. 1494) | The vertex-attribute term of the bake-set predicate (§4.7.3) |
| `ShaderPropertiesModel.engineFlags` (l. 1412) | Read only for the two bake-time flags in the predicate (`separateAo`, `oldLighting`); behavior remains each owner's per P3 §3.1 |
| `PackLoadFailure` + closed codes (ll. 1565–1570) | Sanitized failure summaries in the selection screen; every failure leaves shaders off |
| Consumer rules (ll. 1572–1575) | I-1/I-2/I-3 in §4.1 |
| `ProfileModel.infer(OptionState)` (§4.8 ll. 1228–1231) | The profile label (§4.5.1) |
| Column formula and its counting rule (§4.3 ll. 938–943) | §4.3.4 |
| Tooltip split and `TooltipSeverity.WARNING` (§4.3 l. 914) | §4.3.5 |
| Threading and immutability guarantees (§7 ll. 1653–1661) | §7 below |
...
**(B) Phase 7 — the reload lifecycle (soft dependency, §G5.3 item 3).** `PHASE_7_DOC.md` is not
verified (§0.3), so this design is written against Phase 7's *spec* in DESIGN.md ll. 1850–1886.
...
2. **Phase 7 owns pack/dimension transitions and publishes new configurations on them.** Assumed
   from the dimension-switch lifecycle (ll. 1879–1882) and confirmed independently by P3 §11.4
   ll. 1967–1970, which is verified text. `FULL` delegates to this.
...
**Request 1 — Phase 3: close the option-model publication surface.** P3 §5.1 l. 1410 *names*
`OptionConfiguration` (`OptionCatalog`, immutable `OptionState`, profiles/screens/sliders/lang) and
l. 1411 names both codecs, but — unlike `ProgramStateModel` (ll. 1471–1489), `ResourceRequirements`
(ll. 1491–1509) and `CustomTextureSpec` (ll. 1449–1467), each closed with executable shapes and
declared ordering — these rows bind **behaviors only**. Four specifics are load-bearing here:
...
**Request 2 — Phase 3: close where persisted per-pack options enter a load.** P3 §4.3 l. 935 states
*"Reload merges persisted values into a new immutable `OptionState`"*, but the binding
`PackLoadRequest` (§2.2 ll. 378–385, §5.1 ll. 1541–1546) carries only the **global**
...
closed without knowing which:
  ...
  *Cost note:* adding a request field is **not** a change to `PackConfiguration`'s record-component
  set, so by P3 §5.3 ll. 1596–1598 it does not by itself increment `CURRENT_SCHEMA_VERSION`. It is
...
| Ambiguous option | Rendered non-interactive with its reported locations in the tooltip, excluded from `*` (§4.3.3). Phase 3 already disabled it (P3 §6 l. 1640) | 2a |
| Cyclic profile or subscreen reference | Phase 3 ignores the cyclic edge and retains the rest (P3 §6 l. 1641); the navigation stack is additionally depth-bounded at 32 (§4.3.1) | 2a |
...
| **Per-pack or global persistence write fails** | Warn; **retain the in-memory state**; keep the pending set intact so the user can retry; never turn the pack off, never lose the edit. Matches P3 §6 l. 1645 | 2a |
| `PackFrontEnd.load` returns `Failed` | Show the sanitized summary and the failure code; the screen stays open with `(off)` selected; the prior configuration is not replaced. Phase 3 guarantees the caller is left shaders-off (P3 §5.1 l. 1570) | 4 |
| Discovery returns no candidates, or the shaderpacks directory is invalid | The list shows `(off)` and `(internal)` only, plus the attributed diagnostic; `discover` never throws (P3 §5.1 ll. 1522–1525) | 4 |
...
| `version.<mcver>` unmet | Warning badge, pack stays off, configuration remains inspectable (row C-1; P3 §6 l. 1643) | 4 |
...
| `ReloadCoordinator` absent or not yet installed at bootstrap | Reload triggers are inert and log once; the GUI still opens, still reads, still persists. A GUI that cannot reload is degraded, not broken | 2a |
...
| `PackFrontEnd.discover` / `load` | **Not Phase 12's choice.** Phase 3 states both may run off the render thread and that `load` has no GL affinity (P3 §7 ll. 1653–1655); publication is the caller's atomic reference operation (l. 1659). Phase 7 owns which thread executes a drained request |
| Persistence writes | Serialized per pack identity by Phase 3 (P3 §7 l. 1660); a writer never mutates the active configuration |
...
- `screenColumns_boundary18Yields2And19Yields3` and `..._27Yields3And28Yields4` — the exact
  widening boundary with expansion-produced options counted (App F.4 l. 1480; P3 §4.3 ll. 940–943).
...
- `engineSettings_roundTripThroughGlobalCodec` and
  `engineSettings_unknownKeyIsPreservedForRoundTrip` — P3 §5.1 ll. 1553–1555.
- `engineSettings_absentOwnerListRendersInertIdentityEntry` — §4.6.2's fallback.

**Structural**
- `model_schemaVersionMismatchIsRejectedBeforeDerivation` — I-3, P3 §5.3 l. 1593.
...
| `D-P12-3` | Expansion order is `OptionCatalog` order; fallback is a stable sort on name by unsigned UTF-8 byte order | I-4 requires a total order; the fallback matches P3's own ordering vocabulary (P3 §5.1 l. 1465) and is locale-independent | §4.3.3, §5.4 req. 1(c) |
| `D-P12-4` | Ambiguous options are excluded from `*` but render disabled when explicitly placed | Phase 3 disables them (§4.3 ll. 909–911); auto-placing permanently inert rows in every pack's main screen is noise, while an author who *placed* one should see it and its locations | §4.3.3 |
...
| `D-P12-7` | An out-of-list current value is displayed and retained; the first cycle moves to index 0 | P3 §4.3 l. 930 forbids constraining a safe current value to the advertised list. Rewriting it on open would persist a change the user never made | §4.4.2 |
| `D-P12-8` | Re-run `discover` before acting on any selection | `PackCandidateId` is valid only against the latest generation (P3 §5.1 ll. 1530–1533); the alternative is an avoidable `INVALID_SELECTION` | §4.6.1 |
...
| `D-P12-10` | Engine settings that shadow an App F.1 flag are **tri-state** (`DEFAULT`/`ON`/`OFF`) | App F.1 l. 1448 gives the in-game setting priority "where both exist", and P3 §3.1 l. 684 leaves the resolution to the behavior owner. Tri-state is that rule made executable, so the owner needs no private convention | §4.6.2 |
| `D-P12-11` | `worldRendererReload` is **predicated** on the bake-set, not unconditional | The reference reloads renderers on every apply and every selection (§3.4) — correct but coarse. Chunk re-meshing is the most expensive act a shader GUI can trigger, nothing contract-visible depends on it (§G4.2), and the predicate is written generally so it survives P3 §11.5 item 2 being revisited | §4.7.3 |
...
1. **PD §14 l. 712 overstates the reference's tooltips.** It reports "lang-file tooltips with
   ...
   §11.5 item 2.
2. **`PHASE_3_DOC.md` l. 2052 is stale.** Its closing note reads *"Phase 3 v1 is **not verified**
   pending a fresh whole-document review"*. That text was written at round 33; round 34 then
   returned literal `PASS` with `Interface changed: no`
   (`docs/phase3/reviews/PHASE_3_REVIEW_34.md` ll. 46–48). **Ruling:** Phase 3 is treated as
   **verified** on the review evidence, which is what §G1.3 defines the state by; the doc's own
   trailing status line has not caught up. Reported so a verify session reading that line does not
   conclude this build session consumed an unverified dependency.
...
- **Final integration review (§G5.3 item 4, l. 681)** — "P4's version-counter invalidation vs P12's
  reload paths" is already on the checklist; add the P12 → P7 `ReloadCoordinator` edge, which is an
  interface Phase 12 exposes to a phase that does not depend on it and which per-phase verification
...
10. `[v0.4]` Implement `EngineSettingsModel` with the seven entries, the tri-state semantics, and
    the global round-trip through `GlobalShaderOptionsCodec`. Hook: every `engineSettings_*` test.
11. `[v0.4]` Implement `PackSelectionModel` over `discover`, including sentinel ordering (consumed,
    not re-derived), status/kind badges, the `version.<mcver>` badge, and the stale-generation
    guard. Hook: every `selection_*` test.
...
14. `[v0.4]` Implement the three platform bindings: the `InputEvent.KeyInputEvent` F3+R observer
    ...
    `ClientCommandHandler` registration for `/reloadShaders`, and the
    `ISelectiveResourceReloadListener`. Hook: `reload_resourceReloadRequestsReacquireOnly` plus a
    manual chord check.
...
*Build session complete per §G1.1 step 3. This document is unverified: it awaits a §G1.2 verify
session, which must not share this session's context. Phase 7's soft-dependency assumptions
(§5.3(B)) carry a standing §G1.3 fix-up obligation.*
