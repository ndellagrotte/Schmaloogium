...
| 36 — `PHASE_3_REVIEW_36.md` | l. 270 `PASS-WITH-CORRECTIONS` | l. 272 `yes` | superseded |
| 62 — P3 §0.67 record (`PHASE_3_DOC.md` ll. 5577–5581) | `PASS-WITH-CORRECTIONS` resolved to verified under §G1.3 | `no binding §5 … change` | **latest — verified** |
...
recorded **and no §5 change outstanding**. Round 36 left a §5 change outstanding, so Phase 3 was
**not verified at consumption time**. P3's latest round is now R62: its `PASS-WITH-CORRECTIONS`
resolves to verified under §G1.3 with no §5 change outstanding, and P3's own closing ledger
records exactly that (`PHASE_3_DOC.md` ll. 5577–5581, §0.67); doc and reviews now agree, and
...
| candidate-006 — §2.2's public declarations are incorporated into their §5.1 rows (P3 §5.1 ll. 3423–3430) | §5.2's `PackFrontEnd.discover`, `PackFrontEnd`/`PackLoadRequest`/`PackLoadResult` and `EngineOptionData` rows; it does **not** cover the `OptionConfiguration` row, so §5.4 request 1(a) stands |
...
Because Phase 7's §5 is published regardless of its verification state, that flag is immediately actionable rather than hypothetical: a
...
6. **Two dependency closing-status paragraphs were read outside the assigned extents.**
   `PHASE_3_DOC.md` ll. 5577–5581 and `PHASE_7_DOC.md` ll. 4776–4778, each a single closing
...
1. C-1 — the P1/P3 line coordinates cited across §§0.2/1.2/2.1/3/4/5/6/8/11 were resolved against
   ...
   ownership sentences (P3 §4.3 l. 2262; P3 §11.4 l. 5175) (pre-§0.66 coordinates). No semantic edit, no §5 change, no new
...
| Persistence **formats** and both codecs (`OptionPersistenceCodec`, `GlobalShaderOptionsCodec`), ISO-8859-1 escaping, atomic write mechanics | **Phase 3** (§4.3 ll. 2310–2312). We own only *when* they are invoked (P3 §4.3 l. 2312) |
...
```
:engine  com.schmaloogium.engine.config      ← P1 §2.1 l. 1770 assigns this package "Phase 3 (+12)"
...
:mod     com.schmaloogium.mod.gui            ← P1 §2.1 l. 1799 assigns this package "Phase 12"
...
named. The `.internal` rule and seam constraints C-1…C-4 (P1 §5.1 l. 5478) apply without exception.
...
| F3-3 | Recognized only when the same file `#ifdef`/`#ifndef`s it | Not re-checked: the presentation model is built from P3's confirmed catalog only (P3 §4.3 ll. 2093–2094) | P3 §4.3 |
| F3-4 | Tooltips split on `". "` | `TooltipBuilder.split` produces one `TooltipLine` per segment (§4.3.5) | `[V:doc]` App F.3 l. 1457 |
| F3-5 | Lines ending `"!"` render **red** | `TooltipSeverity.WARNING` → red in both views; P3 sets the severity (§4.3 l. 2107), Phase 12 chooses the colour (§4.3.5) | `[V:doc]` App F.3 l. 1458; P3 §4.3 l. 2107 |
...
| F3-10 | Ambiguous options (conflicting defaults) are **disabled** | Rendered with `interactive=false`, label suffixed with the ambiguity marker, tooltip carrying P3's reported locations; excluded from `*` expansion (§4.3.3, `[D-P12-4]`) | `[V:doc]` App F.3 l. 1467; P3 §4.3 ll. 2102–2103, §6 l. 4417 |
...
| F3-20 | `sliders=<option list>` renders listed **variable** options as sliders | `SliderOption` emitted instead of `ValueOption` for members of P3's `SliderSet`; unknown/non-variable entries are diagnosed by P3 and simply do not become sliders (§4.4.3, `[D-P12-5]`) | `[V:doc]` App F.3 l. 1470; P3 §3.2 l. 1762 |

### 3.2 RESEARCH Appendix F.4 — profiles and screens

| # | Contract item (App F.4, ll. 1472–1480) | Design element | Provenance |
|---|---|---|---|
| F4-1 | `profile.NAME=` tokens `OPTION` / `!OPTION` / `OPTION:value` / `OPTION=value` | Applied as a batch into the pending set on profile click (§4.5.2), from P3's expanded constraint list | `[V:doc]` App F.4 ll. 1474–1475; P3 §3.2 l. 1763 |
| F4-2 | `profile.OTHER` copy, cycle-guarded | Consumed post-expansion; P3 ignores the cyclic edge and diagnoses (§6) | `[V:doc]` App F.4 l. 1475; P3 §4.8 ll. 2967–2968 |
| F4-3 | `!program.<name>`, optionally dimension-prefixed | **Not actuated here** — retain explicit profile intent independently of options; P7 evaluates P3's new-configuration program state and P4 consumes it. No GUI registry mutation (§4.5.2) | `[V:doc]` App F.4 ll. 1475–1476; P3 §3.2 l. 1763 |
...
| F4-7 | Entry: option names | `SwitchOption`/`ValueOption`/`SliderOption` (§4.3.2) | `[V:doc]` App F.4 l. 1478 |
...
| F4-11 | Entry: `*` — all unplaced options | **Phase 12's** deferred expansion, specified completely in §4.3.3 (`[D-P12-1]`, `[D-P12-2]`, `[D-P12-3]`) | `[V:doc]` App F.4 l. 1479; deferred to us by P3 §4.3 ll. 2320–2321 and §4.8 ll. 2969–2970 |
...
|---|---|---|---|
| C-1 | App F.2 `version.<mcver>=<edition>` → pack-list warning | Candidate row renders `CompatibilityStatus.REQUIRES_NEWER_EDITION` as a warning badge + tooltip; the pack stays off (§4.6.1) | `[V:doc]` App F.2 l. 1452; P3 §4.8 ll. 2895–2896, §6 l. 4420 |
| C-2 | §4.7 "only changed options persist to `shaderpacks/<pack>.txt`" | `apply()` writes exactly the changed set through `OptionPersistenceCodec`; Phase 12 computes the change set, Phase 3 owns the format (§4.7.2) | `[V:observed]` §4.7 ll. 603–605; P3 §4.3 ll. 2310–2312 |
| C-3 | §4.7 "global engine settings in `optionsshaders.txt`" | The seven entries round-trip through `GlobalShaderOptionsCodec` → `EngineOptionData`, with their exact wire text bound in §4.6.3 | `[V:observed]` §4.7 l. 605; P3 §5.1 ll. 3497–3500 |
...
| C-9 | §4.7 failure handling: invalid programs **delete themselves and fall back through backup chains** | **Not actuated here** — Phase 4 deletes the program and walks the backup chain (P1 §6 l. 5597). Phase 12's obligation is the §G4.5 GUI channel: the per-program compile/link/validate diagnostics Phase 4 emits accumulate in Phase 1's `SHADER_GUI` store and are rendered by §4.9, driver log included | `[V:observed]` §4.7 ll. 612–614; DESIGN.md §G4.5 ll. 584–585 (per-program compile errors → the shader GUI, "per RESEARCH.md §4.7") ; P1 §4.9.4 l. 5031, §6 l. 5597 |
| C-10 | §4.7 failure handling: capability gate → chat error | Not ours — `UserChannel.CHAT`. The GUI additionally shows the resulting shaders-off state (§4.9) | `[V:observed]` §4.7 ll. 612, 615; P1 §4.9.4 l. 5038 |
...
| `(internal)` pack entry | **No reference.** Pintonium hardcodes `isInternal()` false (PD §7.1) and its list is a plain directory scan (ll. 230–248). We render P3's `PackCandidateKind.INTERNAL` sentinel from `discover` (§4.6.1) | PD §7.1; P3 §5.1 l. 3404; §2.2 l. 784 |
...
- `PackConfiguration.options()` — Phase 3's `OptionConfiguration`: the `OptionCatalog`, the
  immutable `OptionState`, and the profiles / screens / sliders / lang models (P3 §5.1 l. 3413);
- `PackConfiguration.compatibility()` and `PackConfiguration.pack()` — for the selection screen's
  status row (P3 §5.1 l. 3408).
...
- **I-1 — no re-parsing.** Phase 12 never opens a pack file, never rescans directives, never
  reinterprets properties, and never bypasses the materializer. P3 §5.1 l. 4038 states this as a
...
- **I-2 — no mutation.** `PackConfiguration` and `OptionState` are deeply immutable (P3 §7
  l. 4445). An edit produces a *pending overlay*, never an in-place change. A reload publishes a
  new configuration (P3 §5.1 l. 4039).
...
(P3 §6 l. 4418). The navigation stack is additionally depth-bounded at 32 as a defence in depth;
...
(P3 §4.8 ll. 2969–2970), and *"Phase 12 expands `*` first, then passes the complete retained
screen-array length as `expandedSlotCount`"* (P3 §4.3 ll. 2320–2321).
...
**ambiguous**. Ambiguous options are disabled by Phase 3 (P3 §4.3 ll. 2102–2103); auto-placing a
...
list" valid "with one warning, per D-P3-14" (P3 §4.3 ll. 2259–2260). A pack option can therefore hold
...
- `sliders=` naming an unknown or non-variable option is diagnosed by Phase 3 (P3 §3.2 l. 1762) and
...
selection UI (P3 §5.1 l. 3404). Consumed properties, all of them Phase 3's:

- Result ordering is already correct: `Off` and `Internal` first, then filesystem candidates in
  Phase 3's deterministic order (P3 §5.1 l. 3404). **Phase 12 does not re-sort.**
- `PackCandidateKind` is closed — `OFF`, `INTERNAL`, `DIRECTORY`, `ARCHIVE` (P3 §2.2 l. 784) — and
  each gets a distinct row icon/prefix.
- `PackCandidateStatus` is closed — `AVAILABLE`, `UNREADABLE`, `UNSAFE`, `LIMIT_EXCEEDED`
  (P3 §2.2 l. 785). Non-`AVAILABLE` candidates render disabled with the status and the candidate's
  attributed diagnostics in the tooltip.
- Display names are sanitized by Phase 3 and are **never** accepted back as paths (P3 §4.1 l. 1888;
  §5.1 ll. 3469–3470). The view keeps them as display strings only.
- `PackCandidateId` is opaque and is valid only against the latest discovery generation for that
  directory; a later discovery supersedes it (P3 §5.1 l. 3404, ll. 3925–3930). The selection screen holds
...
- A `LOAD` that fails returns exactly one `Failed` and leaves the caller in shaders-off (P3 §5.1
  ll. 4031–4032). The screen shows the sanitized summary and stays open with `(off)` selected.
...
(P3 §4.8 ll. 2895–2896, §6 l. 4420), so the user can read the reason rather than face a silent
no-op (row C-1).
...
(P3 §4.3 ll. 2310–2312). *"Phase 12 owns apply/discard timing; codecs never mutate a published
configuration"* (P3 §4.3 l. 2312), and *"It owns discard timing and global-setting UX"*
(P3 §11.4 l. 5222). This section is the answer to
exactly that.
...
Phase 12's screen renders" (P1 §4.9.4 l. 5031). P1 §5.3 l. 5570 names Phase 12 as the
channel consumer and §11.4 ll. 6747–6748 states plainly: *"the `SHADER_GUI` diagnostic channel and its
per-pack error store exist; your screen is the sink."* P1 §9 l. 5899 tags the routing `v0.4` with
...
- Activating it opens a scrollable diagnostics panel: severity, subsystem, message, and — for
  per-program compile/link/validate failures — the driver log Phase 4 attached (P1 §6 l. 5597).
  Long driver logs are scrollable and selectable, never truncated to a single line.
- A **capability-gate** failure is `ERROR`/`CHAT` (P1 §4.9.4 l. 5038), so it does not reach this
...
already designed for exactly this case (P1 §10.2 ll. 5994–5995). **Blocking defect, restated:** the
template declares only `modCompileOnly`/`modRuntimeOnly` — *no `modImplementation` configuration
exists* — and P1 §12 item 43 is the fix (P1 §11.3 item 2, §5.3 l. 5577). P1 §11.4 l. 6749 predicted
...
lives in `:engine` and satisfies seam constraints C-1…C-4 (P1 §5.1 l. 5478).
...
|---|---|
| Module layout + §2.1 package table (§5.1 l. 5477) | `engine.config` is granted to "Phase 3 (+12)" (l. 1770) and `mod.gui` to Phase 12 (l. 1799) — §2.1. No new package is requested |
| Seam constraints C-1…C-4 and the package-placement/`.internal` rule (§5.1 ll. 5478–5479) | The model/view split in §2.1 |
| `EngineDiagnostic`, `DiagnosticSeverity`, `UserChannel`, `DiagnosticReporter` (§5.3 l. 5570, which names **12** as "GUI is a channel consumer") | §4.9 — the `SHADER_GUI` store is our sink |
| `Log` / `Logs` / the fixed channel list (§5.3 ll. 5568–5569) | GUI-side warnings during model build and persistence |
| Mod-dependency declaration mechanics (P1 §5.3 l. 5577, which indexes P1 §4.2.6, P1 §4.8.4 and P1 §11.3 item 2) | `[D-P12-14]` in §4.10.3 and request 4 below |
| Naming: `mod_id = schmaloogium`, root package `com.schmaloogium` (§5.1 l. 5486) | Type placement throughout |
| SPDX header convention + `THIRD-PARTY.md` mechanism (§5.3 l. 5578) | Applied to any incorporated LGPL-3.0 view code (§0.5) |
...
template declares only `modCompileOnly`/`modRuntimeOnly` — *no `modImplementation` configuration
exists* — and P1 §12 item 43 is the fix; P1 §11.4 l. 6749 anticipated that it "will likely bite you
...
| Ambiguous option | Rendered non-interactive with its reported locations in the tooltip, excluded from `*` (§4.3.3). Phase 3 already disabled it (P3 §6 l. 4417) | 2a |
| Cyclic profile or subscreen reference | Phase 3 ignores the cyclic edge and retains the rest (P3 §6 l. 4418); the navigation stack is additionally depth-bounded at 32 (§4.3.1) | 2a |
...
| **Per-pack or global persistence write fails** | Warn; **retain the in-memory state**; keep the pending set intact so the user can retry; never turn the pack off, never lose the edit. Matches P3 §6 l. 4430 | 2a |
...
| Capability gate fails at init | `ERROR`/`CHAT` (P1 §4.9.4 l. 5038). The GUI shows the resulting off state honestly (§4.9) | 4 |
...
| `PackFrontEnd.discover` / `load` | **Not Phase 12's choice.** Phase 3 states both may run off the render thread and that `load` has no GL affinity (P3 §7 ll. 4440–4442); publication is the caller's atomic reference operation (l. 4446). Phase 7 owns which thread executes a drained request |
| Persistence writes | Serialized per pack identity by Phase 3 (P3 §7 ll. 4447–4448); a writer never mutates the active configuration |
| `SHADER_GUI` diagnostics | Phase 1 hops `CHAT` and `SHADER_GUI` deliveries to the client thread (P1 §7 l. 5623); the panel reads an already-client-thread-confined store |
...
- `engineSettings_roundTripThroughGlobalCodec` and
  `engineSettings_unknownKeyIsPreservedForRoundTrip` — P3 §5.1 ll. 3547–3548.
...
**Structural**
- `model_schemaVersionMismatchIsRejectedBeforeDerivation` — I-3, P3 §5.3 ll. 4212–4213.
...
| `D-P12-4` | Ambiguous options are excluded from `*` but render disabled when explicitly placed | Phase 3 disables them (§4.3 ll. 2102–2103); auto-placing permanently inert rows in every pack's main screen is noise, while an author who *placed* one should see it and its locations | §4.3.3 |
...
| `D-P12-7` | An out-of-list current value is displayed and retained; the first cycle moves to index 0 | P3 §4.3 l. 2260 forbids constraining a safe current value to the advertised list. Rewriting it on open would persist a change the user never made | §4.4.2 |
| `D-P12-8` | Re-run `discover` before acting on any selection | `PackCandidateId` is valid only against the latest generation (P3 §5.1 l. 3404, ll. 3925–3930); the alternative is an avoidable `INVALID_SELECTION` | §4.6.1 |
