...
| H-FBO-03 `Framebuffer` | `func_147605_b(I,I)V` (`createFramebuffer`) HEAD/RETURN and `func_147608_a()V` (`deleteFramebuffer`) HEAD | PHASE_5_DOC §4.8 piece 1: suppress the vanilla depth-renderbuffer allocation, restore `field_147619_e` (`useDepth`), attach the `mod.glue` depth texture (`DepthTexture`) and delete it with the owning framebuffer; H-FBO-01/02 then observe a sampleable texture | `CORE`; evidence line `H-FBO-03 main depth renderbuffer replaced by texture`; without it no P5 estate can borrow `depthtex0` (added 2026-09-11 with the composition root) |
...
|---|---|---|
| `FrameDriver` / `FrameHookSink` | render-thread transaction owner; closed open/step/scope/finish/abort results; idempotent normal/finally finish; `resetShadersOffLatch()` (fix-up 2026-09-11): the composition root re-opens admission when it installs an accepted replacement publication — the shaders-off latch belongs to the publication whose frame failed, never to the session | vanilla Mixin/event adapter; future Kirino adapter |
