...
**Authored:** 2026-07-24 · **Last revised:** 2026-07-26 (§0.13)
...
date is the most recent of the dates the fix-up addenda in §0.4–§0.13 carry, and each addendum states
its own: §0.4–§0.5 are 2026-07-24, §0.6–§0.10 are 2026-07-25, §0.11, §0.12 and §0.13 are all
2026-07-26. The
...
**§G1.3 status:** this fix-up **altered §5** at four rows — §5.1's provider row and §5.2's
`GLCapabilityProfile`, opaque-handle and non-verbs rows. **No facade verb, result type, value type or
declaration changed:** the seven services, the handle types and `ForeignTextureProvider`'s signature are
byte-for-byte what round thirteen reviewed; what moved is *contract stated in prose about types that
...
everything downstream stay blocked until a **fourteenth** verify session returns (§G5.3). What that
session inherits: the four deferred notes, two of which (V13-7, V13-8) this fix-up's own edits enlarge
rather than leave alone — recorded as such in the Resolutions; and the widened key space and bind-only
...
- **Legal arguments:** `TextureService.bindToUnit(int, TextureHandle)` and `DebugService.label(…)`.
  **Illegal:** `allocate`, `setParameters`, `upload`, `generateMipmap` and `delete`. A backend
  **rejects** a foreign handle on those five with `IllegalArgumentException` rather than silently
  performing them — the same class of caller programming error as use-after-delete above, and caught by
  §6's last row rather than reaching vanilla state. §12 item 22b carries it as a review obligation.
...
illegal to `allocate`/`setParameters`/`upload`/`generateMipmap`/**`delete`** — and they are **outside**
...
*End of PHASE_1_DOC.md. Per §G1.1 the build session stopped here. **Thirteen** verify sessions have
since run — `PHASE_1_REVIEW_1.md` through `PHASE_1_REVIEW_13.md`, each returning PASS-WITH-CORRECTIONS —
and **ten** fix-up sessions: the first applied round one's F-1 … F-12 (§0.4); the second applied
...
(V13-4's mipmap misattribution in §3) being the round's one genuine miss by twelve prior rounds.
...
**This fix-up altered §5**, at four rows, so it does **not** close the phase. Three of round thirteen's
four corrections reach it: the provider's key space is **two disjoint vocabularies** — App B.3's sampler
names, Phase 5's, and `minecraft:` resource locations, Phase 13's — because the identifier Phase 13
holds is App F.5's *source* side and not a *destination* sampler name (V13-2, §5.1's provider row); a
foreign `TextureHandle` is **bind-only and outside the handle-lifetime rule**, so Phase 5's
drop-and-re-create must not reach it — a delete over the unit table destroys Minecraft's block atlas
(V13-3, §5.2's opaque-handle row); and `supportsMipmapGeneration()` is consumed by **Phase 7** and
**Phase 8**, not by a "Phase 5 composite-mipmap policy" that `DESIGN.md` gives no phase (V13-4, §3's row
and §5.2's profile row). The fourth, V13-1, deletes an `[A]`-tagged claim RESEARCH.md ll. 526 and
545–546 refute, and reaches §5.2's non-verbs row because Phase 7's `final`-to-vanilla bind is a **v0.1**
obligation the row had printed as a someday one. **No facade verb, result type, value type or
declaration changed:** the seven services, the handle types and `ForeignTextureProvider`'s signature are
byte-for-byte what round thirteen reviewed — what moved is contract *stated in prose* about types that
already existed, which is the whole shape of this round. So §G1.3's *"re-verify only if §5 changed"*
trigger **fires**; `PHASE_1_DOC.md` is **not verified**, it is **not** a
valid dependency input, and Phase 2, Phase 3 and everything downstream are blocked until a
**fourteenth** verify session returns (§G1.3, §G5.3). On the repository state, which round twelve found
misdescribed here and round thirteen re-derived and confirmed: **§G0.4 steps 1, 2 and 4 and the `v10` → `v11` roll all landed after the round-eleven
...
§4.5.2a added is no longer unreviewed, and neither is what §0.12 added to them; what is unreviewed now
is what §0.13 added — the widened key space, the bind-only rule, and the corrected grounds in §3, §4.7.4
and §4.12 — which round seven's rule prices exactly as honestly: unreviewed material yields findings in
