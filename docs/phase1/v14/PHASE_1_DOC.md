...
**Authored:** 2026-07-24 · **Last revised:** 2026-07-26 (§0.14)
...
date is the most recent of the dates the fix-up addenda in §0.4–§0.14 carry, and each addendum states
...
*End of PHASE_1_DOC.md. Per §G1.1 the build session stopped here. **Fourteen** verify sessions have
since run — `PHASE_1_REVIEW_1.md` through `PHASE_1_REVIEW_14.md`, each returning PASS-WITH-CORRECTIONS —
and **eleven** fix-up sessions: the first applied round one's F-1 … F-12 (§0.4); the second applied
...
other (V14-2) a round-six-created row whose ownership clauses no round had examined.
...
**This fix-up altered §5**, at three rows, so it does **not** close the phase. Both of round fourteen's
corrections reach it: the foreign-handle rule is now stated **by closure over §4.7.4** — legal to
`bindToUnit` and `DebugService.label` and to nothing else that accepts a `TextureHandle`, with
`FramebufferService`'s `attachColor`/`attachDepth`/`copyDepthToTexture` named illegal and the last
named destructive, because its `dst` is written into (V14-1, §5.1's provider row and §5.2's
opaque-handle row); and the per-program `alphaTest.<prog>`/`blend.<prog>` routing is re-derived to
`DESIGN.md`'s **3 (parse), 4 (apply), 7 (execute)** — deleting the unanchored "per-buffer routing"
clause, since App F.7's form has no per-buffer axis — which adds **Phase 4** to §5.2's `StateService`
consumers (V14-2, §3's two rows). **No facade verb, result type, value type or declaration changed:**
the seven services, the handle types and `ForeignTextureProvider`'s signature are byte-for-byte what
round fourteen reviewed — what moved is contract *stated in prose* about types that already existed,
which is the whole shape of this round. So §G1.3's *"re-verify only if §5 changed"* trigger **fires**;
`PHASE_1_DOC.md` is **not verified**, it is **not** a valid dependency input, and Phase 2, Phase 3 and
everything downstream are blocked until a **fifteenth** verify session returns (§G1.3, §G5.3). On the
repository state, which round twelve found misdescribed here and round thirteen re-derived and
confirmed: **§G0.4 steps 1, 2 and 4 and the `v10` → `v11` roll all landed after the round-eleven
...
stop-and-report if a briefed coordinate disagrees with this document. What §0.13 added is no longer
unreviewed; what is unreviewed now is what §0.14 added — the closure-stated bind-only rule and the
re-derived App F.7 rows — which round seven's rule prices exactly as honestly: unreviewed material
yields findings in proportion to its size, not to the document's maturity.*
