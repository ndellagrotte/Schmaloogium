...
**Authored:** 2026-07-24 · **Last revised:** 2026-09-08 (§0.36)
...
date is the most recent of the dates the addenda in §0.4–§0.36 carry, and each addendum states
...
2026-08-03; §0.24–§0.29 are 2026-09-07; §0.30–§0.36 are 2026-09-08. The
...
1's owner and trigger; RESEARCH.md §3.2 and App A.3 (`countInstances`), App D.4 (`instanceId` as
an `int` uniform; GLSL 120 has no `gl_InstanceID`, engine-general OpenGL knowledge), §4.5 and App B.2 (the shadow pass's
...
the ownership; RESEARCH.md **§4.2** (the ~90 built-in uniforms and the 60 slots), **§4.4** (the
per-frame flow V9-1's effectiveness limb turns on), **App A.1** (what the 60 actually counts; the 43-slot figure is DESIGN.md's, RC2 l. 570) and
...
(1409–1525) for one term; RESEARCH.md ll. 488, 527, 546–547, 814, 1186, 1229–1246, 1399 (App E's column
heading), 1417, 1481–1484; `FinalPassRenderer.java` l. 289, behavioural observation only (§G11.2). No
...
before `final` (RESEARCH.md ll. 546–547) and GL bindings are sticky, so a `final` that does not rebind
...
hits); RESEARCH.md ll. 419–437 (§3.6.7's `PER_BUFFER_BLENDING`, l. 430) and 1508–1516 (App F.7 —
`blend.<prog>` carries no per-buffer axis, l. 1517); §4.7.4's declarations swept for `TextureHandle`
...
glue, mixins, and capture agent (`docs/phase7/v1/PHASE_7_DOC.md:4032`); all four already exist
...
"mod.mixin.textures" (`docs/phase13/v1/PHASE_13_DOC.md:1950`–`:1951`). Those three exact homes
...
`MaterializedSource` has no whole-source compatibility result (`:1429-1433`).
...
### 0.36 Attempt-10 review-37 corrections — 2026-09-08

Review 37 returned PASS-WITH-CORRECTIONS with five documentary corrections and no §5 change
(`docs/phase1/reviews/PHASE_1_REVIEW_37.md`). This fix-up applies them: the header revision date
and addendum-date enumeration are refreshed (C1); §1.2's legacy-geometry scope row now states the
D-P3-68 `GeometrySourceRequest.None/PreserveNative` grant with current-schema receipt D-P1-65
instead of the stale ungranted sentence (C2); the closing history recounts thirty-seven verify
sessions and the fix-up/maintenance sessions through this one (C3); and §3's absent-`final` row
is repointed to RESEARCH.md `:1141` (C4), the mipmap row's `:1186` already standing in the
received bytes (C5). Dated addenda keep their historical request anchors: §0.15's and D-P1-39's
Phase 4 request coordinates and §0.25's Phase 3/4 contract quotes describe the bytes those
sessions read, and the live grants they preceded are recorded by D-P1-39/D-P1-44, §5.2, §11.4 and
P3 §0.61. Inputs read: review 37, the RC2 Phase 1 specification and §G1.3, and the cited
RESEARCH coordinates. No §5 row, facade verb, result/value type, package grant or interface
region changes; R37's verdict and all earlier verdicts remain historical and fresh whole-document
review and final integration are still required. No build, test, implementation or PASS claim
accompanies these documentation edits. `v14` stays.
...
| Legacy geometry source preservation/translation and source-map/fingerprint production | **Phase 3**; complete core translation is not established by its two-span contract. Native-preserving materialization is owner-granted through D-P3-68's `GeometrySourceRequest.None/PreserveNative`, current schema deferred to P3 §5.3 (§5.2, §11.4; receipt D-P1-65) |
...
| **Absent `final` terminal** — passthrough copy, not fixed-function selection | RESEARCH.md App **A.1**, `docs/research/v1/RESEARCH.md:1141` | Downstream final-pass execution owns the passthrough copy. It does **not** invoke `ShaderService.useFixedFunction()` merely because `final` is absent | `[V:doc]` |
| Geometry programs may declare the **ARB form** (`#extension GL_ARB_geometry_shader4` + `const int maxVerticesOut = N`) | `docs/research/v1/RESEARCH.md:213-215` says layout qualifiers **or** the ARB form; `:1161` names the legacy configuration; `:771` lists internal translation as an opportunity | `ShaderService.configureLegacyGeometry` supplies the native pre-link parameters through engine enums (§4.7.4, D-P1-44). Core-form sources retain their layout path. Phase 3 legacy-preserving output is owner-granted through D-P3-68's `PreserveNative` route and Phase 4's consumer migration is receiver-adopted, both unverified (§5.2, §11.4); neither this facade grant nor P3's preservation grant alone establishes dual-form conformance | `[V:doc]`; route is D-P1-44 |
...
- **Program selection has two explicit modes, and neither mode is the whole state barrier.**
  ...
  `docs/phase4/v1/PHASE_4_DOC.md:1773`–`:1808`). The facade does not silently absorb that policy
...
`docs/phase4/v1/PHASE_4_DOC.md:2843`–`:2847`; `ShaderService.useFixedFunction()` now serves it, so it is
...
*is* unserved here: RESEARCH.md **l. 527** names the `final`-to-vanilla render, and `DESIGN.md` puts it
...
| D-P1-39 | **`ShaderService` exposes `useFixedFunction()` as the explicit no-program selection operation, additive to `use(ProgramHandle)`.** It binds program zero inside the backend; callers may not encode the state as `use(null)`, a magic/sentinel handle, or an engine-visible integer. Recording uses zero-argument `shaders.useFixedFunction`, and the existing generic replay assertions inspect that exact operation | RESEARCH.md Appendix A.1 has fixed-pipeline terminals for `<none>` and absent shader roots plus absent-`final` passthrough, while the verified Phase 1 surface exposed only handle-bearing `use`. Phase 4 obeyed the dependency rule by requesting the missing verb in `docs/phase4/v1/PHASE_4_DOC.md:2843`–`:2847` instead of inventing it. A distinct zero-argument operation preserves C-1 and the handle-lifetime model: program zero is backend state, not an engine object, so it is neither created/deleted nor eligible for `noUseAfterDelete()`. Phase 4 retains every policy decision in `ProgramStateBarrier`; this verb performs selection only (§0.15, §3, §4.7.4–§4.7.5) |
...
exactly as `docs/phase4/v1/PHASE_4_DOC.md:1773`–`:1808` specifies. This addition does **not** resolve
...
These §5-changing grants received the attempt-11 whole-document review; IR-01's disposition is recorded 2026-09-09.
...
*End of PHASE_1_DOC.md. Per §G1.1 the build session stopped here. **Thirty-seven** verify sessions have
...
twenty-two returned literal PASS, round twenty-three returned literal PASS, round twenty-four
returned PASS-WITH-CORRECTIONS, round twenty-five returned the literal PASS that verified the §0.23
surface, round twenty-six returned PASS-WITH-CORRECTIONS, and rounds twenty-seven through
thirty-seven each returned PASS-WITH-CORRECTIONS whose corrections §0.29–§0.36 record — and
**thirty-four** fix-up/maintenance sessions: the first applied
...
and the **twenty-first** through **twenty-fifth** carried the Phase 7/13 package-placement, Phase 4
legacy-geometry, IR integration-reconciliation and IR-18 prepared-submission amendments and R26's
fix-up with its commissioned foundation grants (§0.24–§0.28); the **twenty-sixth** recorded the
settled producer receipts (§0.29); the **twenty-seventh** and **twenty-eighth** applied the R27
native-array isolation and the P3 schema-ownership receipt (§0.30–§0.31); and the
**twenty-ninth** through **thirty-first** applied the attempt-5/6/7 corrections (§0.32–§0.34);
...
The **review-36 fix-up** appended §0.35's record of that resolution plus §3's two RESEARCH.md
coordinate repoints; and this **thirty-fourth** session applies review thirty-seven's five
corrections as documentary edits (§0.36).
...
whole-owner review (`docs/phase1/reviews/PHASE_1_REVIEW_38.md`, frozen SHA-256
`05abca4b2fa4c0b7dc1bf3a8f4a13ac8543b5b5e2d64134d3a471217153abb47`) returned
PASS-WITH-CORRECTIONS — 0 blocking, 2 corrections (F-38-1, F-38-2), 1 note (N-38-1).
This fix-up wave applied every correction and recorded resolutions in the review file;
no §5 bytes changed and no §5 change is outstanding. Per §G1.3 the document is
**verified** and is a valid dependency input. The superseded §0.25/§0.28/§0.35 status
records above stand as history. No implementation, validation command or conformance
result accompanies these edits. `v14` stays.
