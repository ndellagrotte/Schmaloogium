...
> **REV4:** **Status:** v2.0-RC4 "ID-EVIDENCE", 2026-08-03. This file is
> `docs/design/v2.0-RC4/DESIGN.md` and is the **unadopted RC4 candidate overall**. It follows
> v2.0-RC3 (`docs/design/v2.0-RC3/DESIGN.md`) in revision history, but authoring this
> candidate migrates no downstream governance: target manifests and phase headers retain
> their declared revisions until §G0.4 is executed for the selected phase.
> **REV4:** **What changed (RC3 → RC4):** the Phase 9 summary no longer presents PD §8.1's
> tag expansion as production-proven on 1.12.2. In this checkout that implementation is
> modern-only; the 1.12.2 path still requires RESEARCH.md §3.6.8's ore-dictionary-style shim
> and entries-before-tags rule. No phase redesign or adoption is performed.
> **REV3 baseline:** v2.0-RC3 "OCULUS", 2026-07-26, lives at
> `docs/design/v2.0-RC3/DESIGN.md`. It followed v2.0-RC2 in revision history; at RC3
> authoring, RC2 governed Phase 1 from its §0.11 onward and v1.1 governed Phase 2.
...
| **REV4:** Area | RC3 | RC4 |
|---|---|---|
| Candidate status | RC3 is adopted by later phase documents while older phases retain their declared revisions | **REV4:** RC4 is a new, unadopted candidate; no target manifest or phase header is repointed by authoring it |
| PD §8.1 evidence boundary | Groups tag expansion with several resolution behaviors described as “all working in production” | **REV4:** production evidence is limited to the applicable non-tag behaviors; `%` tag parsing and lookup are modern-only in this checkout |
| 1.12.2 tag support | The adjacent modern-risk table requires a shim, but the Phase 9 summary can be read as contradicting it | **REV4:** Phase 9 explicitly retains RESEARCH.md §3.6.8's ore-dictionary-style shim and entries-before-tags priority rule |
| Closing audit | REV1–REV3 change audits | **REV4:** adds this candidate's narrow additions audit and end marker |
...
- **The verified injection timeline (REV1 evidence, corrected by REV5 — start here).** Pintonium ships a battle-tested
  ...
  | 3a | same site, **AFTER** the first clear | **REV5:** post-clear buffer/frame preparation: begin-frame lifecycle, resize check, shadow/main clear preparation, frame-update notification, custom uniforms, and `begin_*` passes; rebind default FB; **do not capture matrices here** |
  | 3b | `renderWorldPass`, INVOKE `EntityRenderer.setupCameraTransform(FI)V` **AFTER** | **REV5:** capture the now-installed gbuffer model-view/projection matrices from `ActiveRenderInfo`, publish them to Phase 6 exactly once for the frame, then permit matrix-dependent draw scopes |
  ...
  ordinal-0 `GlStateManager.clear` INVOKE site is a proven stable anchor** for post-clear
  buffer/frame preparation in 1.12.2. **REV5:** it is not the matrix-capture anchor: the main
  camera matrices are captured only after `setupCameraTransform(FI)V` returns.
...
- `schlorbium-project/files.txt` (**REV5:** cross-check: 17 of Appendix E's 18 classes appear in
  OF's replacement list; `net.minecraft.client.shader.Framebuffer` does not, so its resolved
  vanilla call sites stand without that independent corroboration).
...
both spike specs complete with fallbacks. **REV1/REV5:** the original 7-row Pintonium timeline and
the split 3a/3b anchors are adopted per row (with Cleanroom-patch/SRG validation cited) or deviation justified; rows with no
...
- **Alias resolution** (§3.7): Phase 3's parsed grammar → resolved lookup tables against
  ...
  metadata ids, property predicates, still↔flowing fluid aliases, **first-writer-wins
  precedence** (`putIfAbsent`, order-significant), and OF-legacy numeric fallback when no
  `block.properties` exists — working production references for those applicable
  behaviors. **REV4 — tag-evidence qualification:** PD §8.1's tag expansion is
  modern-only in this checkout: the 1.12.2 parser produces entry selectors, not `%` tag
  selectors, so its common resolver's tag branch is not production proof for 1.12.2.
  Phase 9 must still provide RESEARCH.md §3.6.8's ore-dictionary-style tag shim and preserve
  the entries-before-tags priority rule.
...
| Original 7-row reference timeline; clear-ordinal-0 preparation anchor; deferred-trigger cross-validation; composite-at-TAIL + stricter guarantee; sky/weather/clouds reference-free; OQ-3/OQ-4 annotations | P7 |
...
**REV4:** additions audit (every RC3→RC4 change, verified placed; each site carries a
literal `**REV4:**` marker):

| **REV4:** Addition/correction | Placed in |
|---|---|
| RC4 identity, own path, unadopted-candidate status, and narrow change statement | **REV4:** header/status block |
| Revision highlights (RC3 → RC4) matched to the actual additions | **REV4:** top highlights table |
| Adoption readiness updated for RC4 and the canonical target-profile/selectors workflow | **REV4:** G0.4 |
| Do-not-modify list extended to RC4 | **REV4:** G1.1 |
