# Schmaloogium — Design Document (Pintonium Revision 1)

> **Status:** v2.0 "PINTONIUM REV1", 2026-07-25. This file is
> `docs/design/v2.0-RC1/DESIGN.md`; it supersedes v1.1
> (`docs/design/v1.1/DESIGN.md`, kept for history) but has not yet been adopted — every
> phase doc and review to date verifies against v1.1, which is what the `/verify-loop`
> harness reads. Authored from [RESEARCH.md](../../research/v1/RESEARCH.md) (source of
> truth, 2026-07-24) and
> [PINTONIUM_DESIGN.md](../../reference/pintonium/v1.0/PINTONIUM_DESIGN.md) (`PD`, the
> Pintonium mining report, 2026-07-25).
> **What changed:** the Pintonium codebase — a legally clean (LGPL-3.0), *working*
> Celeritas+Oculus fork running demanding shader packs on 1.12.2 LWJGL3 — is now a
> primary reference implementation for Phases 3–14. This revision adds rules of
> engagement for it (§G11), corrects or strengthens specific phase designs against its
> verified findings, and records the decisions its divergences force. RESEARCH.md remains
...
| `Pintonium/…` | the Pintonium repo (Celeritas + Oculus/Iris fork with 1.12.2 shader support) — LGPL reference implementation and 1.12.2 field report. **Rules of engagement: §G11** (search exclusions, licensing, do-not-inherit lists). `Pintonium/DESIGN.md` is a copy of *this* document, not Pintonium docs; the analysis is `Pintonium/PINTONIUM_DESIGN.md` (`PD`). |
...
| OQ-20 | open | **G8/S5** + **P1** (seam hardness requirement) | Highest-weight strategic risk. **REV1:** Pintonium's ~3-service seam driving 4 MC backends (PD §2) is further evidence seam-hardness is achievable |
...
1. **`celeritas-shader-refactor/` is a stale full copy of the repo** — untracked, not in
   the build, already diverged. Exclude it from all searches; never treat its files as
   current. (Prefer `git grep` or path-scoped searches over naive recursive greps.)
2. Root-level `mixins.vintagefix.json` / `mixins.vintagefix.late.json` are stray files
   referenced by no source set.
3. `Pintonium/DESIGN.md` is a copy of *our* design document, not Pintonium documentation.
...
- **`mod.glue` provider-interface completeness check** (REV1): PD §2's service inventory —
  ...
  design; note deliberately-excluded items (we have one backend, not four).
...
service-based seam drove four radically different MC backends from one shader core.
...
- **Compile/link flow** (§4.2): per-file — Phase 3 front-end output → compile → attach →
  ...
  **11/12/13/14** (Iris numbering, `ProgramCreator.java:21-25`, PD §3.2/§18) — that works
...
- **PD §8 (whole).** Pintonium `common-shaders/.../shaderpack/IdMap.java` +
  `forge122/.../materialmap/VintageBlockMaterialMapping.java` (LGPL — §G11 rules).
...
- **Constant-attribute delivery candidate (REV1 — decision recorded, §G11.4):** Pintonium
  ...
  vertex-attribute *constant* for the draw (verified at `RenderGlobalMixin.java:459`,
  PD §8.2). On a compat context, any attribute whose array is disabled reads the
...
- **PD §14 (GUI + persistence paragraphs).** Pintonium
  `forge122/.../gui/VintageShaderPackSelectionScreen.java` +
  `VintageShaderPackOptionsScreen.java` (LGPL — §G11 rules).
...
*End of design document (Pintonium Revision 1).*
