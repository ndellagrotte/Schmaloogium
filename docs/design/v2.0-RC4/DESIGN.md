# Schmaloogium — Design Document (**REV4:** ID-evidence clarification, v2.0-RC4)
...
### G0.4 Adoption procedure and readiness (**REV4:** RC4 status)

**REV4:** This revision is the unadopted candidate overall. Per `docs/MOVES.md`'s
...
uses RC2, Phase 2 uses v1.1, and Phases 3–9 use RC3. Pointing any phase at RC4 without
re-derivation would not necessarily error; plausible text exists at the old coordinates.
Adopting RC4 for a downstream phase means, in order:

1. **REV4:** **Re-derive the candidate's complete selector/pin set from this file's own
...
2. **REV4:** **Dry-run the selected target**, then update the operator source map only if
   engine/prompt behavior changed. Update `docs/MOVES.md`'s `DESIGN.md` collision and
   version-label records for every adoption.
3. **REV4:** **Migrate only the phase selected for adoption.** Assess RC4's delta against
...
**REV4:** Until all four steps are complete for a phase, that phase keeps its declared
...
- **Do not modify** RESEARCH.md, any design revision (`docs/design/v1.1/DESIGN.md`,
  ...
  `docs/design/v2.0-RC3/DESIGN.md`, `docs/design/v2.0-RC4/DESIGN.md` — **REV4:** the
...
*End of design document (**REV4:** ID-evidence clarification, v2.0-RC4).*
