# Schmaloogium — Design Document (Pintonium Revision 2)

> **Status:** v2.0-RC2 "PINTONIUM REV2", 2026-07-26. This file is
> `docs/design/v2.0-RC2/DESIGN.md`; it supersedes v2.0-RC1
> (`docs/design/v2.0-RC1/DESIGN.md`) and v1.1 (`docs/design/v1.1/DESIGN.md`), both kept
> for history — but has not yet been adopted: every phase doc and review to date verifies
> against v1.1, which is what the `/verify-loop` harness reads, so **v1.1 remains the
> governing design until the §G0.4 adoption procedure is executed**. Downstream state at
> this writing: PHASE_1_DOC is at v10 with eleven reviews, the last
> (`PHASE_1_REVIEW_11.md`) PASS-WITH-CORRECTIONS with no `## Resolutions` recorded — so
> Phase 1 is **not verified** under §G1.3; PHASE_2_DOC is at v1, built under §G1.1 with
> zero verify sessions, its §0.3 item 6 disclosing the §G5.3 gating exception it was
> built under. Authored from
> [RESEARCH.md](../../research/v1/RESEARCH.md) (source of truth, 2026-07-24),
> [PINTONIUM_DESIGN.md](../../reference/pintonium/v1.0/PINTONIUM_DESIGN.md) (`PD`, the
> Pintonium mining report, 2026-07-25), and REV1's text.
...
### G0.4 Adoption procedure and readiness (REV2)

This revision is an unadopted candidate (RC). Per `docs/MOVES.md`'s version-label rule, a
design revision keeps its `-RC` label exactly until a downstream document adopts it;
adoption is a deliberate maintainer operation, never a side effect. **The trap is
silent:** every phase doc and review to date cites v1.1 *by line number*, and the
`/verify-loop` harness reads v1.1 through hardcoded coordinates — pointing any of them at
this file without re-derivation would not error, it would feed sessions the wrong text
(the standing `docs/MOVES.md` warning). Adopting this revision means, in order:

1. **Re-derive every line pin** in `.claude/workflows/verify-loop.js`: the `DESIGN` path
   constant, the ~16 section→line mappings, and each `PHASE_FACTS` row's `spec`/`docGate`
   range — computed from *this* file's headings, never shifted from v1.1's numbers.
...
3. **Migrate the phase docs.** PHASE_1_DOC and PHASE_2_DOC were built and reviewed
   against v1.1. For each: assess this revision's delta against the sections that doc
   actually consumed; route claim-touching deltas through a §G1.3 fix-up session (the
   review-file `## Resolutions` convention), **not** a rebuild — unless the delta
   invalidates the doc's §5 cross-phase interfaces, which re-opens verification per
   §G1.3's re-verify rule. The REV2 changes were shaped to be fix-up-sized (rung 2a
   explicitly grandfathers the five-rung mapping; the §G1 path conventions codify what
   the docs already do).
...
Until all four steps are complete, sessions treat v1.1 as governing and this file as the
candidate it is; a session briefed with line coordinates that do not match its assigned
document stops and reports rather than guessing.
...
1. **Read and understand your phase.** Mandatory reading, in order:
   - All of Part I (G0–G11) of this document, plus your own phase spec in Part II.
...
- **Do not modify** RESEARCH.md, any design revision (`docs/design/v1.1/DESIGN.md`,
  `docs/design/v2.0-RC1/DESIGN.md`, `docs/design/v2.0-RC2/DESIGN.md` — **REV2:** the
  list now names all three), PINTONIUM_DESIGN.md, or another phase's doc. Propose changes
  to any of them in your doc §11 ("requested upstream changes").
...
- **Trust and reuse freely:** pack front-end structure (include graph, options, profiles,
  ...
  stareval's evaluator architecture (license gate: §G11.2 rule 3), the mixin
...
*End of design document (Pintonium Revision 2).*
