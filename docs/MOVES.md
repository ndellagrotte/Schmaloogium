...
The cost of that trade is one collision: **two files are now named `DESIGN.md`.** They are different
documents. See the warning below.
...
## ⚠️ Two files named `DESIGN.md`

| | `docs/design/v1.1/DESIGN.md` | `docs/design/v2.0-RC1/DESIGN.md` |
|---|---|---|
| Lines | 1,586 | 2,300 |
| Status | **the governing design** | unadopted revision |
| Phase 1 spec at | l. 585 | l. 825 |
| Phase 2 spec at | l. 662 | l. 929 |
| §G1.2 at | l. 118 | l. 170 |
...
**Re-pointing the harness at v2.0-RC1 would not raise an error — it would silently feed every agent
the wrong text.** At v2.0-RC1 ll. 649–652 the doc-gate lens reads §G9's template, which looks
entirely plausible. Moving to v2.0-RC1 means re-deriving every line number first.
...
Everything else resolves. The acceptance check:

```bash
grep -rhoE 'docs/[A-Za-z0-9._/-]+\.md' docs --include='*.md' | sort -u | while read p; do
